    ...
    def test_all(self):
        ...
        for i in range(0, 4301, 100):
            ...
            if len(block_content) > 0:
                self.assertEqual(expected_output[str(i)]['block_count'], len(block_content))
                self.assertEqual(expected_output[str(i)]['transaction_count'], sum([block['transaction count'] for block in block_content]))

                tx_id_list = reduce(lambda x,y :x+y ,[block['transaction id list'] for block in block_content])
